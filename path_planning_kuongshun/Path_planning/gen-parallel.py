...
def main():
    """
    @brief Main entry point: create and run multiple robot instances in PARALLEL.
    """
    ...
    logger.info("Generator (PARALLEL): Starting multiple robot instances")
    ...
    logger.info(f"Configured {len(controllers)} robot instances. Starting parallel execution...")
    ...
    with ThreadPoolExecutor(max_workers=len(controllers)) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.error(f"Robot {robot_id} encountered an error: {e}")

    logger.info("Generator (PARALLEL): All robots finished")
