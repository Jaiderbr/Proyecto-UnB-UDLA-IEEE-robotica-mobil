...
class Corobeu():
    ...
    def connect_CRB(self, port):
        ...
        returnCode, robot = sim.simxGetObjectHandle(clientID, 'Turtlebot_Robot_Koungshun___2_Acrylic_Base_Plate_2_1',sim.simx_opmode_blocking)  # Obteniendo el objeto "Pioneer_p3dx" de v - rep(Robot Movil)
        returnCode, MotorE = sim.simxGetObjectHandle(clientID, 'left_motor', sim.simx_opmode_blocking)  # Obteniendo el objeto "Pioneer_p3dx_leftmotor" de v-rep (motor izquierdo)
        returnCode, MotorD = sim.simxGetObjectHandle(clientID, 'rigth_motor', sim.simx_opmode_blocking)  # Obteniendo el objeto "Pioneer_p3dx_rightMotor" de v - rep(motor derecho)
    ...
    def Speed_CRB(self, U, omega, lock_stop_simulation, signed, error_phi):
        ...
        L = 13  # Distance between the wheels
        R = 2.25 # Wheel radio
